...
m5$save_object(file = modfit_name) #save stan output so you don't have re-run model
#m5 <- readRDS(modfit_name)


print(n=500, as_tibble(
      m5$summary(c("mean", "sd", "rhat", "ess_bulk"),
                 variables=c(
                              "lp__",
                              "Lsigma_h",
                              "Lsigma_w",
...
#str(post5, list.len=100)
saveRDS(post5, modpost_name)
#post5 <- readRDS(modpost_name)
#post5$"GrpOffset[1]"
...
fitmodb <- m5$draws(format = "draws_list", inc_warmup = TRUE) # array format keeps chains separate for plotting
#str(fitmodb)
