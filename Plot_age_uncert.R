...
pdf(file="./Plots/Sim_age_uncert_fh.pdf",
    height=10, width=10)
...
par(mar = c(0, 0, 0, 0), oma = c(5, 5, 6, 4)) #margins for indiv plot, oma for outer margins (bottom, left, top, right)
  ...
  paramList <- readRDS(parlist_name1)


  Nsims <- 10   # number of trajectories to simulate
  ...
  obsheights <- Com.fem$Height #readRDS("post6_sim10x101_heights.RDS") #substitute for heightdata definition below
  post <-  readRDS(modpost_name1)

  N <- nrow(Com.fem)                    # number of observations
  x <- seq(from=(0), to=(26+0.75), by=0.1)        # sequence for plotting functions, include gestation in age

  #str(post, list.len=ncol(post))

  for ( n in 1:N ){
      if ( Com.fem$Ethnicity[n] == 2 && Com.fem$firstObs[n] != -1) {            # if simulated data and not conception
            estcyr <- mean( pull(post, paste("concepyear[", Com.fem$ID[n], "]", sep="")) ) # posterior mean year of conception
            estage <- pull(post, paste("obsyearout[", n, "]", sep=""))[1] - estcyr  # obsyearout repeats the data for each MCMC sample
            obsheightdata <- obsheights[n]
            #obsheightdata <- pull(post, paste("heightout[", n, "]", sep=""))     # if using the updated m6.stan 
            points(x=estage, y=obsheightdata,
                  lwd=MatPoint_lwd, col="red", cex=MatPoint_cex, pch=MatPoint_pch)
            ...
            lines( x=c( (Com.fem$interview_year[n] - Com.fem$ConcepYear[n]), estage ),
                   y=c(Com.fem$Height[n], Com.fem$Height[n]), lwd=0.5, lty=1, col="black")

      } # if
  } # for N

  text(1, 170, "10 individuals", cex = 1.35, adj=0)


  #set up plot for labels
  ...
  text("Height (cm)", x=-6.5, y=-1.5, srt=90, las=3, cex=1.8)
...
# legend
  ...
  xcoords <- c(0,
               1.7, #1.5 #4.5,  #4 #8.5 # moves third item right
               4.6 #4 #8.5#, #7.5 #24.2 # moves last item right
  ...
  obsheights <- Com.fem$Height #readRDS("post6_sim100x101_heights.RDS") #substitute for heightdata definition below
  post <-  readRDS(modpost_name2)
  ...
  text(1, 170, "100 individuals", cex = 1.35, adj=0)
